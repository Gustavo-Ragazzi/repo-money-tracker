    ...
    with get_connection() as conn:
        rows = conn.execute(
            ...
            SELECT
                ...
                p.created_at,
                l.id,
                l.previous_remaining_money,
                l.collected_money,
                l.total_available,
                pls.purchases_json,
                pls.loans_given_json,
                pls.total_donated,
                pls.total_received,
                pls.previous_remaining,
                pls.post_donation_balance,
                pls.final_balance
            FROM sessions s
            LEFT JOIN
                players p ON p.session_id = s.id
            LEFT JOIN
                levels l ON l.session_id = s.id AND l.number = s.actual_level
            LEFT JOIN
                player_level_state pls ON pls.level_id = l.id AND pls.player_id = p.id
            WHERE s.id = ?
            """,
            (session_id,),
        ).fetchall()
...
def create_session(name: str, player_names: list[str]) -> int:
    with get_connection() as conn:
        ...
        for i, player_name in enumerate(player_names):
            ...
            cursor.execute(
                """
                INSERT INTO players (slot, session_id, name, is_host, created_at)
                VALUES (?, ?, ?, ?, ?)
                """,
                (i + 1, session_id, player_name.strip(), int(i == 0), created_at),
            )

            cursor.execute(
                """
                INSERT INTO player_level_state (
                    level_id,
                    player_id,
                    purchases_json,
                    loans_given_json,
                    ...
                    total_received,
                    previous_remaining,
                    post_donation_balance,
                    final_balance,
                    created_at)
                VALUES (1, ?, null, null, 0, 0, 0, 0, 0, ?)
                """,
                (i + 1, created_at),
            )
